import React from 'react';
import { Link } from 'react-router-dom';

const PrenatalSteps = () => {
  const steps = [
    {
      number: 1,
      title: 'Book Your Appointment',
      description: (
        <>
          <Link to="/appointments" className="text-blue-600 underline font-medium">Schedule a DNA test</Link> or book online to schedule your prenatal DNA test. We’ll explain the process, answer questions, and coordinate sample collection for both parents.
        </>
      ),
    },
    {
      number: 2,
      title: 'Provide Safe, Non-Invasive Samples',
      description:
        'The mother provides a simple blood draw at a certified site, and the alleged father provides a quick cheek swab at our office.',
    },
    {
      number: 3,
      title: 'Partner Laboratory Analysis',
      description:
        'An AABB accredited laboratory we partner with extracts and compares fetal DNA from the mother’s blood with the alleged father’s DNA profile to determine paternity with over 99.9% accuracy.',
    },
    {
      number: 4,
      title: 'Receive Fast, Accurate Results',
      description:
        'Results are typically available in 7–10 business days. Legal tests include notarized documentation suitable for court, immigration, or personal use.',
    },
  ];

  return (
    <section className="bg-white py-16 px-6 md:px-16 lg:px-24">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-12">
          The Prenatal DNA Testing Process — Step by Step
        </h2>
        <div className="grid gap-8 md:grid-cols-2">
          {steps.map((step) => (
            <div
              key={step.number}
              className="flex items-start gap-4 p-6 bg-white rounded-xl border-l-4 border-blue-600 shadow-sm hover:shadow-md transition"
            >
              <div className="text-3xl font-bold text-blue-600">{step.number}</div>
              <div>
                <h3 className="text-xl font-semibold text-gray-800 mb-2">{step.title}</h3>
                <p className="text-gray-600">{typeof step.description === 'string' ? step.description : step.description}</p>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default PrenatalSteps;
