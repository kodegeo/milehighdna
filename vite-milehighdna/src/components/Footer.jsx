// src/components/Footer.jsx
import React from 'react';
import { Link } from 'react-router-dom';
import clsx from 'clsx';
import { MH_FOOTER_DISCLOSURE } from '../constants/aabbComplianceCopy';


const Footer = () => {
  return (
    <footer className="bg-gray-100 text-gray-700 py-12 border-t">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8 text-sm">
        <div>
          <h4 className="text-lg font-semibold mb-4">Mile High DNA Testing</h4>
          <p className="text-gray-600">
            Providing trusted, confidential DNA testing services to families across <a href="https://milehighdnatesting.com/dna-testing-colorado" className="text-blue-600 hover:text-blue-700 transition duration-300">Colorado</a>.
          </p>
        </div>

        <div>
          <h5 className="font-semibold mb-3">Quick Links</h5>
          <ul className="space-y-2">
            <li><Link to="/about" className="text-gray-700 hover:text-blue-600 transition duration-300">About Us</Link></li>
            <li><Link to="/services" className="text-gray-700 hover:text-blue-600 transition duration-300">Services</Link></li>
            <li><Link to="/aabb-accredited-dna-testing-denver" className="text-gray-700 hover:text-blue-600 transition duration-300">AABB standards & partner laboratory</Link></li>
            <li><Link to="/faq" className="text-gray-700 hover:text-blue-600 transition duration-300">FAQ</Link></li>
          </ul>
        </div>

        <div>
          <h5 className="font-semibold mb-3">Appointments</h5>
          <ul className="space-y-2">
            <li><Link to="https://calendly.com/milehighdnatesting/30-minute-dna-test-appointment" className="text-gray-700 hover:text-blue-600 transition duration-300">Free Consultation</Link></li>
            <li><Link to="/services" className="text-gray-700 hover:text-blue-600 transition duration-300">Service Appointment</Link></li>
            <li><Link to="/appointments" className="text-gray-700 hover:text-blue-600 transition duration-300">Book Now</Link></li>
          </ul>
        </div>

        <div>
          <h5 className="font-semibold mb-3">Contact</h5>
          <p className="text-gray-600 mb-2">Denver, CO</p>
          <p className="text-gray-600 mb-2">Phone: (720) 900-9342</p>
          <p className="text-gray-600">Email: info@milehighdnatesting.com</p>
        </div>
      </div>

      <div className="mt-12 border-t pt-8 px-4 max-w-4xl mx-auto">
        <p className="text-xs text-gray-600 text-center leading-relaxed mb-6">{MH_FOOTER_DISCLOSURE}</p>
      </div>

      <div className="border-t pt-6 text-center text-xs text-gray-500">
        &copy; {new Date().getFullYear()} Mile High DNA Testing. All rights reserved.
      </div>
    </footer>
  );
};

export default Footer;